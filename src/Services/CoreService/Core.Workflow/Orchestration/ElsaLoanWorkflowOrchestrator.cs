    ...
    ILogger<ElsaLoanWorkflowOrchestrator> logger) : ILoanWorkflowOrchestrator
{
    ...
    public async Task SignalLoanCaseAsync(Guid caseId, string signal, object? payload, CancellationToken ct)
    {
        var instanceId = await loanCaseRepository.GetWorkflowInstanceIdAsync(caseId, ct);
        await SignalAsync(caseId, instanceId, LoanCaseWorkflow.DefinitionId, signal, payload, ct);
    }
        ...
        CancellationToken ct)
    {
        ...
        logger.LogWarning(
            "Elsa bookmark not matched for loan case {CaseId} instance {InstanceId}; resetting workflow instance and bookmarks.",
            caseId,
            instanceId);
        ...
        logger.LogWarning(
            "Elsa workflow could not be resumed for loan case {CaseId} after reset; domain state was still updated.",
            caseId);
    }

...
public sealed class KanbanAppService(
    ...
    ICaseStateManager stateManager) : IKanbanAppService
{
    public async Task<Result<IReadOnlyList<KanbanCaseCardDto>>> GetActionRequiredAsync(CancellationToken cancellationToken)
    {
        ...
        if (access.IsFailure) return Result<IReadOnlyList<KanbanCaseCardDto>>.Fail(access.Error!);
    ...
    {
        var access = EnsureKanbanAccess();
        if (access.IsFailure) return Result<IReadOnlyList<KanbanCaseSummaryDto>>.Fail(access.Error!);
